...
Set `storm_health_damage = true` to replace exposed storm energy drain with `storm_damage * dt` health damage, independent of mass. Health capacity remains `health_per_mass * mass`, so fully healthy creatures' uninterrupted storm survival time scales linearly with mass rather than quadratically. Health mode currently defaults to true; `storm_damage` defaults to 0.4 health/second and allows zero. Nursery and ordinary shelter protection still apply. Health mode requires predation/body mechanics. Damage triggers injury feedback, blocks healing for that step, and can kill; storm deaths produce normal corpses but do not count as predation kills. CLI options are `--storm-health-damage 0|1` and `--storm-damage X`. Checkpoints preserve both settings; older supported checkpoints retain energy-drain mode. Existing health-mode checkpoints resumed with this executable also use the new mass-independent damage rule.
