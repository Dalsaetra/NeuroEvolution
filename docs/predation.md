...
With mass_scaled_energy_capacity enabled (the default for new runs), reserve capacity is energy_capacity × (reproduction_cost + body_energy_per_mass × mass) / (reproduction_cost + body_energy_per_mass). Thus energy_capacity remains the mass-1 reference. The reproduction threshold stays fixed; actual birth cost still depends on the child's mass. Capacity creates no energy: digestion fills it and discards overflow. Energy sensors and replay bars use individual capacity. Older checkpoints retain their fixed capacity.
