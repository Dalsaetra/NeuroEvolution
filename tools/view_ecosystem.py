...
.details-panes .event-list{height:300px}
...
<section class="panel"><div class="panel-head"><h2>Creature inspector</h2><span class="tiny muted" id="selectedStatus">individual brain</span></div><div class="selection"><select id="creatureSelect" aria-label="Select creature"></select><div id="selectedContent"><div class="agent-title"><i class="agent-dot" id="agentDot"></i><span id="agentTitle"></span><span class="tiny muted" id="agentParent"></span></div><div class="small-label">Survival energy</div><div class="energy-track"><i id="energyFill"></i></div><div id="reproductionReserve" hidden><div class="small-label">Reproductive energy <span id="reproductionAmount"></span></div><div class="energy-track"><i id="reproductionFill"></i></div><div class="tiny muted" id="reproductionStatus"></div></div><div class="stats"><div class="stat"><strong id="agentEnergy">—</strong><span>energy</span></div><div class="stat"><strong id="agentAge">—</strong><span>age, seconds</span></div><div class="stat"><strong id="agentGeneration">—</strong><span>generation</span></div><div class="stat"><strong id="agentSpikes">—</strong><span>recorded spikes</span></div><div class="stat"><strong id="agentOffspring">—</strong><span>offspring</span></div><div class="stat"><strong id="agentPosition">—</strong><span>position</span></div></div><div id="bodyTraits" class="subsection tiny" hidden></div><div class="subsection"><div class="small-label">Movement &amp; actions</div><div class="actions" id="actions"></div></div><div class="subsection"><div class="small-label">Lifetime diet · biomass</div><div class="diet" id="diet"></div><div class="diet-labels" id="dietLabels"></div></div></div><p class="empty" id="noCreature" hidden>No creatures remain in this frame.</p></div></section>
...
const foodSources=new Map((M.food_sources||[]).map(source=>[source.id,source]));
function drawFoodSources(){
 const s=worldView.scale;
 for(const source of foodSources.values()){
  const p=point(source.x,source.y),r=source.radius*s;
  ctx.beginPath();
  if(source.kind==='field'){
   for(let i=0;i<=100;i++){const a=i*Math.PI/50,edge=r*(.82+.10*Math.sin(3*a+source.phase)+.07*Math.sin(5*a-source.phase));const x=p.x+Math.cos(a)*edge,y=p.y-Math.sin(a)*edge;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}
   ctx.closePath();ctx.fillStyle='#7c9b5030';ctx.strokeStyle='#68864080';
  }else{ctx.arc(p.x,p.y,r,0,2*Math.PI);ctx.fillStyle=source.kind==='pod-tree'?'#a1885030':'#66906b30';ctx.strokeStyle=source.kind==='pod-tree'?'#a1885090':'#66906b90'}
  ctx.fill();ctx.lineWidth=1;ctx.stroke();
  if(source.kind!=='field'){ctx.beginPath();ctx.arc(p.x,p.y,Math.max(2,s*.3),0,2*Math.PI);ctx.fillStyle='#765436';ctx.fill()}
  ctx.font='10px Segoe UI';ctx.textAlign='center';ctx.fillStyle='#465c39';
  ctx.fillText(`${source.kind==='field'?'Field':source.kind==='pod-tree'?'Pod tree':'Fruit tree'} ${source.id}`,p.x,p.y-r-3);
 }
}
...
function updateInspector(){const frame=F[index],select=$('creatureSelect'),c=frame.creatures.find(c=>c.id===selected);select.replaceChildren();if(!c&&selected!==null){const o=document.createElement('option');o.value=selected;o.textContent=`Creature ${selected} · absent from this frame`;select.append(o)}frame.creatures.forEach(c=>{const o=document.createElement('option');o.value=c.id;o.textContent=`Creature ${c.id} · generation ${c.generation??0} · ${fmt(c.energy,1)} energy`;select.append(o)});if(selected!==null)select.value=selected;$('selectedContent').hidden=!c;$('noCreature').hidden=!!c;$('noCreature').textContent=frame.creatures.length?'This creature is not alive in this frame. Select another creature above.':'No creatures remain in this frame.';$('selectedStatus').textContent=c?`${c.controller||M.controller||'spiking'} controller`:'no active selection';if(c){$('agentDot').style.background=color(c.id);$('agentTitle').textContent=`Creature ${c.id}`;$('agentParent').textContent=c.parent?`parent ${c.parent}`:'founder';$('agentEnergy').textContent=fmt(c.energy,1)+' / '+fmt(num(c.max_energy)||energyMax,1);$('energyFill').style.width=`${clamp(c.energy/(num(c.max_energy)||energyMax),0,1)*100}%`;$('energyFill').style.background=c.energy<(num(c.max_energy)||energyMax)*.25?'var(--red)':'var(--green)';$('reproductionReserve').hidden=!M.funded_reproduction;const rt=num(c.reproduction_target),re=num(c.reproductive_energy),rc=num(c.reproduction_cooldown_remaining);$('reproductionAmount').textContent=fmt(re,1)+(rt>0?' / '+fmt(rt,1):'');$('reproductionFill').style.width=(rt>0?clamp(re/rt,0,1)*100:0)+'%';$('reproductionFill').style.background='var(--green)';$('reproductionStatus').textContent=(rc>0?'Cooldown '+fmt(rc,1)+' s':rt>0?(re>=rt?'Fully funded; waiting for birth':'Gestating'):'No pending offspring')+' · Allocation '+fmt(100*(c.reproduction_allocation??0.5),0)+'%';$('agentAge').textContent=fmt(c.age,1);$('agentGeneration').textContent=c.generation??0;$('agentSpikes').textContent=fmt(c.spikes);$('agentOffspring').textContent=fmt(c.offspring);$('agentPosition').textContent=`${fmt(c.x,1)}, ${fmt(c.y,1)}`;$('agentPosition').style.fontSize='13px';$('bodyTraits').hidden=!M.predation;$('bodyTraits').textContent=`Mass ${fmt(c.mass,2)} / Eye separation ${fmt(num(c.eye_separation_degrees),1)}° / Vision ${fmt(num(c.fov_degrees)||num(M.fov_degrees)||150,1)}° / Carnivory ${fmt(100*num(c.carnivory))}% / Mutation scale ${fmt(c.mutation_scale??1,2)} / Health ${fmt(c.health,1)} of ${fmt(c.max_health,1)}`;$('actions').replaceChildren(action('Drive',c.forward),action('Speed',c.speed,num(c.maximum_speed)||num(M.max_speed)||1.5),action('Turn',c.turn,num(M.max_turn_rate)||Math.PI),action('Forage',c.forage),action('Call',c.call),...(M.predation?[action('Attack',c.attack)]:[]));const eaten=c.eaten||[0,0,0,0],total=eaten.reduce((s,v)=>s+num(v),0);$('diet').replaceChildren();$('dietLabels').replaceChildren();['Graze','A','B','Pod','Meat'].forEach((label,i)=>{const segment=document.createElement('span');segment.style.background=foodColors[i];segment.style.width=total?`${num(eaten[i])/total*100}%`:'0';$('diet').append(segment);const l=document.createElement('span');l.textContent=`${label} ${fmt(eaten[i],1)}`;$('dietLabels').append(l)})}drawBrain(c);if(sensorTab)drawSensors(c);}
...
function potentialSamples(creatureId,neuron){
 return F.map(frame=>{
  ...
  const input=phase?creature?.observation?.[neuron]:null;
...
function drawPotentialHistory(data){
 ...
 const sensorSamples=samples.filter(p=>p.input!==null);
 if(phase){const input=samples[index]?.input;caption.textContent=`Sensor input: ${input==null?'not recorded':`${input}${input===0?' (off)':''}`} · ${caption.textContent} Blue: sensor input (0–1). Stored phase can remain when input is off; it is not ongoing activation. Recorded phase may round to 1.`}
 let lo=Infinity,hi=-Infinity;[...valid.map(p=>p.value),...thresholds.map(p=>p.threshold),...sensorSamples.map(p=>p.input)].forEach(v=>{lo=Math.min(lo,v);hi=Math.max(hi,v)});
 ...
 // Sampled sensor values are distinct from the input neuron's retained phase.
 c.strokeStyle='#397cba';c.lineWidth=1.3;c.beginPath();let previousInput=null;
 samples.forEach(p=>{if(p.input===null){previousInput=null;return}if(previousInput){c.lineTo(x(p.time),y(previousInput.input));c.lineTo(x(p.time),y(p.input))}else c.moveTo(x(p.time),y(p.input));previousInput=p});c.stroke();
...
function render(){drawStatChart();const f=F[index];$('timeline').value=index;$('time').textContent=`${clock(f.time)} / ${clock(lastTime)}`;$('population').textContent=fmt(f.creatures.length);$('populationNote').textContent=`${population[0]} at recording start · ${fmt(f.time,1)} s`;$('meanEnergy').textContent=fmt(energies[index],1);$('birthDeath').textContent=`${fmt(f.totals.births)} / ${fmt(f.totals.deaths)}`;$('foodConsumed').textContent=fmt(f.totals.consumed_biomass,1);$('maturations').textContent=fmt(f.totals.maturations);$('weather').textContent=f.weather||'calm';$('weather').className=`weather ${f.weather||'calm'}`;$('worldStatus').textContent=f.capacity_limited?'Population capacity reached · births paused':!f.creatures.length?'Extinct':'';drawWorld();updateInspector();drawChart('populationChart',population,'#387961');drawChart('energyChart',energies,'#b18a42');updateEvents();}
