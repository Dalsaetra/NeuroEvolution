...
function drawBrain(c){
 ...
 const height=Math.max(300,24*Math.max(...groups.map(g=>g.length))+36);brain.style.height=`${height}px`;
 ...
 synapses.forEach(e=>{const a=points.get(e.pre),b=points.get(e.post);if(!a||!b)return;bctx.beginPath();bctx.moveTo(a.x,a.y);bctx.lineTo(b.x,b.y);bctx.strokeStyle=num(e.weight)<0?'#b46e7170':'#668b8160';bctx.lineWidth=.7;bctx.stroke()});
...
brain.addEventListener('mousemove',e=>{const r=brain.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top;let nearest=null,best=7;brainPoints.forEach(p=>{const d=Math.hypot(p.x-x,p.y-y);if(d<best){nearest=p;best=d}});if(nearest){const c=F[index].creatures.find(c=>c.id===selected),d=brainData(c),n=nearest.i,inputs=num(d.inputs)||87,outputs=num(d.outputs)||5,label=n<inputs?(M.input_labels[n]||`Input ${n}`):n>=d.neurons.length-outputs?`Action ${['move','turn left','turn right','forage','call'][n-(d.neurons.length-outputs)]||n}`:`Recurrent neuron ${n}`;showTip(`${label} · ${n<inputs&&M.calibrated_io?`phase ${fmt(d.potentials?.[n],3)} / 1 · rate divisor ${fmt(nearest.n.threshold,3)}`:`V ${fmt(d.potentials?.[n],3)} / threshold ${fmt(nearest.n.threshold,3)}`}`,e.clientX,e.clientY)}else $('tooltip').hidden=true});
