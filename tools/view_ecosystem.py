...
def read_replay(path: Path) -> dict[str, Any]:
    ...
    source = path / "ecosystem.jsonl" if path.is_dir() else path
    ...
    return {"metadata": metadata, "frames": frames, "name": source.parent.name}
...
<section class="panel"><div class="panel-head"><h2>The habitat <span class="muted tiny" id="dimensions"></span></h2><span class="weather" id="weather">Calm</span></div>
...
<div class="legend"><span><i class="swatch" style="--c:#689951"></i>Ground forage</span><span><i class="swatch" style="--c:#dc8262"></i>Fruit A</span><span><i class="swatch" style="--c:#6c90bd"></i>Fruit B</span><span><i class="swatch" style="--c:#ba9548"></i>Food pod</span><span><i class="swatch square" style="--c:#a7c5b3"></i>Shelter</span><span><i class="swatch square" style="--c:#536358"></i>Wall</span></div>
</section>
...
const foodColors=['#689951','#dc8262','#6c90bd','#ba9548'];
const kindIndex={'graze':0,'fruit-a':1,'fruit-b':2,'fruit_a':1,'fruit_b':2,'pod':3};
...
let worldView={scale:1,x:0,y:0},brainPoints=[],sensorTab=false;
const world=$('world'),ctx=world.getContext('2d'),brain=$('brain'),bctx=brain.getContext('2d');
const population=F.map(f=>f.creatures.length),energies=F.map(f=>f.creatures.length?f.creatures.reduce((s,c)=>s+num(c.energy),0)/f.creatures.length:0);
...
function drawWorld(){const {w,h}=fit(world);ctx.clearRect(0,0,w,h);worldView.scale=Math.min(w/M.width,h/M.height);worldView.x=(w-M.width*worldView.scale)/2;worldView.y=(h-M.height*worldView.scale)/2;const s=worldView.scale;ctx.drawImage(terrain,worldView.x,worldView.y,M.width*s,M.height*s);const frame=F[index];if(frame.weather==='storm'){ctx.fillStyle='#5e7b961a';ctx.fillRect(worldView.x,worldView.y,M.width*s,M.height*s)}const chosen=frame.creatures.find(c=>c.id===selected);if(chosen&&$('showVision').checked)drawVision(chosen);
...
function drawBrain(c){const {w,h}=fit(brain);bctx.clearRect(0,0,w,h);brainPoints=[];const data=brainData(c),neurons=data?.neurons||[];$('brainEmpty').style.display=neurons.length?'none':'block';if(!neurons.length){$('brainCaption').textContent='Enable brain recording to inspect membrane potentials and spikes.';return}const inputs=num(data.inputs)||num(M.inputs)||87,outputs=num(data.outputs)||5,minX=Math.min(...neurons.map(n=>num(n.x))),maxX=Math.max(...neurons.map(n=>num(n.x))),minY=Math.min(...neurons.map(n=>num(n.y))),maxY=Math.max(...neurons.map(n=>num(n.y))),spiked=new Set(data.spiked||[]);brainPoints=neurons.map((n,i)=>({x:20+(num(n.x)-minX)/Math.max(.01,maxX-minX)*(w-40),y:16+(num(n.y)-minY)/Math.max(.01,maxY-minY)*(h-32),i,n}));if(maxX===minX||maxY===minY){brainPoints=neurons.map((n,i)=>{const layer=i<inputs?0:i>=neurons.length-outputs?2:1,start=layer===0?0:layer===1?inputs:neurons.length-outputs,count=layer===0?inputs:layer===1?Math.max(1,neurons.length-inputs-outputs):outputs;return {x:25+layer*(w-50)/2,y:14+(i-start+.5)/count*(h-28),i,n}})}const synapses=data.synapses||[];synapses.forEach(edge=>{const a=brainPoints[edge.pre],b=brainPoints[edge.post];if(!a||!b)return;bctx.beginPath();bctx.moveTo(a.x,a.y);bctx.lineTo(b.x,b.y);bctx.strokeStyle=num(edge.weight)<0?'#b46e7133':'#668b8130';bctx.lineWidth=.45;bctx.stroke()});brainPoints.forEach(p=>{const threshold=num(p.n.threshold)||1,potential=num(data.potentials?.[p.i]),input=p.i<inputs,output=p.i>=neurons.length-outputs;const r=input?2:output?4.3:3.2;bctx.beginPath();bctx.arc(p.x,p.y,r,0,Math.PI*2);bctx.fillStyle=spiked.has(p.i)?'#e4b43d':input?'#75a694':output?'#b37d67':`rgba(50,112,96,${.3+clamp(potential/threshold,0,1)*.7})`;bctx.fill();if(spiked.has(p.i)){bctx.beginPath();bctx.arc(p.x,p.y,r+2,0,Math.PI*2);bctx.strokeStyle='#e4b43d66';bctx.lineWidth=1;bctx.stroke()}});$('brainCaption').textContent=`${neurons.length} neurons · ${synapses.length} synapses · ${spiked.size} firing now. ${c?.brain?.potentials?'Gold = spike; red connections = inhibitory.':'Graph only; activity not recorded in this frame.'}`;}
function drawSensors(c){const container=$('sensors');container.replaceChildren();if(!c?.observation?.length){const e=document.createElement('p');e.className='empty';e.textContent='Sensory inputs were not recorded for this creature.';container.append(e);return}c.observation.forEach((v,i)=>{const row=document.createElement('div');row.className='sensor';const label=document.createElement('span');label.textContent=M.input_labels[i]||`Input ${i}`;label.title=label.textContent;const track=document.createElement('div');track.className='sensor-track';const fill=document.createElement('i');fill.style.width=`${clamp(Math.abs(num(v)),0,1)*100}%`;if(v<0)fill.style.background='#ba7c70';track.append(fill);const val=document.createElement('b');val.textContent=fmt(v,2);row.append(label,track,val);container.append(row)})}
...
function render(){const f=F[index];$('timeline').value=index;$('time').textContent=`${clock(f.time)} / ${clock(lastTime)}`;$('population').textContent=fmt(f.creatures.length);$('populationNote').textContent=`${population[0]} at recording start · ${fmt(f.time,1)} s`;$('meanEnergy').textContent=fmt(energies[index],1);$('birthDeath').textContent=`${fmt(f.totals.births)} / ${fmt(f.totals.deaths)}`;$('foodConsumed').textContent=fmt(f.totals.consumed_biomass,1);$('maturations').textContent=fmt(f.totals.maturations);$('weather').textContent=f.weather||'calm';$('weather').className=`weather ${f.weather||'calm'}`;$('worldStatus').textContent=f.capacity_limited?'Population capacity reached · simulation stopped':'';drawWorld();updateInspector();drawChart('populationChart',population,'#387961');drawChart('energyChart',energies,'#b18a42');updateEvents();}
