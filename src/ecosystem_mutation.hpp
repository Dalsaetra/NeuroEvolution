...
{
    // Archive exploration retains occasional broad changes.
    ...
    mutation.mutate_weight_probability = std::min(1.0, std::max(0.45, mutation.mutate_weight_probability * 2));
    mutation.mutate_neuron_probability = std::min(1.0, std::max(0.35, mutation.mutate_neuron_probability * 2));
    mutation.add_synapse_probability = std::min(1.0, std::max(0.75, mutation.add_synapse_probability * 2));
    mutation.add_neuron_probability = std::min(1.0, std::max(0.35, mutation.add_neuron_probability * 2));
    mutation.add_reciprocal_motif_probability = std::min(1.0, std::max(0.30, mutation.add_reciprocal_motif_probability * 2));
    mutation.remove_synapse_probability = std::min(1.0, std::max(0.10, mutation.remove_synapse_probability * 2));
    return mutation;
}
